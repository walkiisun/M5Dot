...
//mqtt vars 
const char* mqtt_broker = "172.22.114.79";
...
int postureThresh(){
  ...
  else if(roll >= 160 || roll  <= -160){
    direction = 2;
  }
  else if(roll > 15 && roll < 160){
    direction = 1;
  }
  else if(roll < -15 && roll > -160){
    direction = 3;
...
void loop() {
  if(finishCollection == true){
    ...
    sendMQTT(createOutMQTT(officalStart, officalEnd, rowData.pitchArray, macAddress, unitArray[4]));
    delay(delayTime);
    
    sendMQTT(createOutMQTT(officalStart, officalEnd, rowData.postureArray, macAddress, unitArray[5]));
